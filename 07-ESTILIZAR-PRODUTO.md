...
## Usar classes do Bootstrap na view de Produto

O Produto tem os mesmos arquivos que a Categoria, com um campo a mais (`description`) e a associação com `category`. Vamos repetir exatamente a mesma receita: listagem, formulário, novo/editar e detalhes.

---

### Listagem (index)
---
A classe `container` centraliza o conteúdo e dá uma margem lateral — é a base de quase toda página com Bootstrap. Trocamos também o aviso verde por um `alert alert-success`, agrupamos os itens da lista com `list-group` e transformamos o link "New product" em um botão de verdade com `btn btn-primary`.
...
### Formulário (_form)
---
A única diferença para o formulário de Categoria é que aqui temos mais campos: `description` (que usa `textarea` em vez de `text_field`) e `category_id`. Todos seguem a mesma classe `form-control`, com cada campo dentro de uma `div class="mb-3"` para dar espaçamento entre eles.
...
---
Essas duas telas só chamam o formulário, então basta envolver tudo em um `container` e transformar os links de volta em botões.
...
Vamos ligar o servidor (se ele já não estiver rodando) e recarregar a página de produtos para ver o resultado.

```bash
rails server
```

Agora, acesse `http://localhost:3000/products`.

Pronto! A tela de Produto agora usa componentes do Bootstrap: `container`, `list-group`, `card`, `btn` e `form-control`.
